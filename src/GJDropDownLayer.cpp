...
GJDropDownLayer::~GJDropDownLayer() {
	...
	// idk what to write here :flushed:
}
...
void GJDropDownLayer::exitLayer(cocos2d::CCObject* p) {
	...
	// director->getTouchDispatcher()->
	// wtf
	//v4 = cocos2d::CCDirector::sharedDirector(v3);
	//*((*(*v4 + 80))(v4) + 76) = 0;
	// how does it work
...
void GJDropDownLayer::hideLayer(bool instantHide) {
	...
	if (instantHide) {
		...
		auto callback = cocos2d::CCCallFunc::create(this, callfunc_selector(GJDropDownLayer::exitLayer)); // nvm probably no
		//v8 = cocos2d::CCCallFunc::create(this, &stru_284.st_value); // no idea how this works ._.
...
void GJDropDownLayer::registerWithTouchDispatcher() {
	//&this->dwordEC; i assume that its 'this'
...
void GJDropDownLayer::showLayer(bool instantShow) {
	...
	if (instantShow) {
		...
		auto callback = cocos2d::CCCallFunc::create(this, &stru_284.st_size); // &stru_284.st_size what
...
bool GJDropDownLayer::init(const char* title, float height) {
	...
	this->m_endPosition = ccp(0, 0);
	this->m_startPosition = ccp(0, 0); // cocos2d::CCPoint::CCPoint(&v47, 0.0, v45); v45 here is not defined so its probably 0
